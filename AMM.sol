...
//全局变量    
    ...
    function unStakeToken(address _token, uint _amount) public {
        address user = msg.sender;
        require((_amount > 0)&&(_amount < userStakingReserve[user][_token]),"invalid amount");
        IERC20 token = IERC20(_token);
    ...
    function checkPositionInfo(address _user,address _token)public view returns(uint _userEth,uint _userBorrowTokenAmount){

        _userEth = userEthBalance[_user][_token];
        _userBorrowTokenAmount = userBorrowedAmount[_user][_token];
    }
    ...
    function buyToken(address _token) public payable returns (uint amountOut) {
        require(
    ...
    function sellToken(address _token, uint _amount) public returns (uint amountOut) {
        require(
            findLpToken[_token] != address(0),
            "invalid token"
        );
